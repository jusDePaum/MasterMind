<table>
    ...
    if (isset($_SESSION["MasterMindGame"])) {
        ...
        if (count($MasterMind->getMoves()) >= 12) {
        ...
        <td colspan="5">
            ...
            if (isset($_SESSION["MasterMindGame"])) {
                ...
                if ($MasterMind->getGameStatus() === Model_StatutPartie::Victoire) { // Si la partie est gagnée
                    $display = "<div><b>Bien joué !</b> Vous avez gagné en " . count($MasterMind->getMoves()) . " essais ! <a href='Controleur/controleurReset.php'>Rejouer</a>"; //Affichage du texte de victoire
                    echo $display;
                }
                if ($MasterMind->getGameStatus() === Model_StatutPartie::Defaite) { // Si la partie est perdue
                    $display = "<div><b>Dommage !</b> Le code secret était " . $MasterMind->getConfig()[Model_MasterMindGame::CONFIG_CODE_TO_GUESS] . " ! <a href='Controleur/controleurReset.php'>Rejouer</a>"; //Affichage du texte de défaite
                    echo $display;
                }
            }
            ?>
